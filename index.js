...
function proxyRequest(localRequest, localResponse, next) {
  ...
  var parsedUrl = url.parse('http://' + localRequest.url.slice(1));

  // pass along the headers from the original request
  var options = _.assign(parsedUrl, {headers: localRequest.headers});
